...
// quadrupole doesn't bend the design trajectory). At theta=0 the geometry
// is the same exact-fit G4Tubs as BuildQuad(); once rotated, an
// orientation-agnostic G4Orb (bounding sphere of the tube) is used instead
...
G4VPhysicalVolume* DetectorConstruction::Construct() {
  ...
  if (fElement == "Chain") {
    ...
    const MitrayPoleData q1data = MitrayPoleData::Q1();
