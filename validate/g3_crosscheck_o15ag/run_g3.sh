#!/usr/bin/env bash
# GEANT3 side of the 15O(a,g)19Ne cross-check: 8 parallel dsbatch jobs of
...
# Outputs land in <workdir>/j1..j8/{dragon1.root,fort.4 (recoil stop points)} and <workdir>/jg/{endv,tail}.txt;
...
BASE=$HERE/data/g3/dragon_o15ag_10k.ffcards   # k39pg 10k cards with FKIN 20 4.0 4.0

setup() {  # setup <dir> <trig> <seed1> <seed2>
  mkdir -p "$1"; cd "$1"
  for f in "$G"/*.dat "$G"/*.txt "$G"/*.root; do ln -sf "$f" .; done
  ln -sfn "$G/dat" dat; cp "$HERE/o15ag.dat" .
